...
window.addEventListener('DOMContentLoaded', () => {
  ...
  burger();
  popup();
  if (currentUrl === 'index.html') slider();
  if (currentUrl === 'pets.html') pagination();
  ...
  console.log(
    ...
    'расположение и размеры элементов в бургер-меню соответствует макету (центрирование по вертикали и горизонтали элементов меню, расположение иконки): +2\n',
    ...
    'при изменении ширины экрана (от 1280px до 320px и обратно), слайдер перестраивается и работает без перезагрузки страницы (набор карточек при этом может как изменяться, так и оставаться тем же, скрывая лишнюю, и сохраняя при этом описанные для слайдера требования). Слайдер автоматически не добавляет дополнительную информацию при увеличии размера экрана, необходимо 2 раза нажать на стрелки для формирования корректного содержимого, при этом перезагрузки страницы не происходит: +2/4\n\n',

    '3. Реализация пагинации на странице Pets: 32 / 36 / \n',
    ...
    'при изменении ширины экрана (от 1280px до 320px и обратно), пагинация перестраивается и работает без перезагрузки страницы (страница может оставаться той же или переключаться, при этом сформированный массив - общая последовательность карточек - не обновляется, сохраняются все остальные требования к пагинации): 0/4 (перестраивается, но работает корректно только при перезагрузке страницы)\n\n',
